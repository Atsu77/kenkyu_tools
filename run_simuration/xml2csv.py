...
class CSVWriter(NestingHandler):
    ...
    def __init__(self, attrFinder, options):
        ...
        for root in sorted(attrFinder.depthTags):
            ...
            self.outfiles[root].write(options.separator.join(
                map(self.quote, attrFinder.attrs[root])) + u"\n")
...
def main(args=None):
    ...
    if options.validation:
        ...
        if not options.xsd and hasattr(
                options.source,
                "name") and options.source.name.endswith(".gz"):
            # we need to reopen the file because the AttrFinder already read
            # and closed it
